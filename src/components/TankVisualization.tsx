...
export default function TankVisualization({
  ...
  const waterHeight = (waterLevel / 100) * (tankHeight - 80); // 80px for top/bottom margins
  ...
  const getWaterColor = (level: number) => {
    if (level < 20) return { primary: '#ef4444', secondary: '#fca5a5', accent: '#fee2e2' }; // Red
    if (level < 40) return { primary: '#f59e0b', secondary: '#fbbf24', accent: '#fef3c7' }; // Orange
    if (level < 70) return { primary: '#3b82f6', secondary: '#60a5fa', accent: '#dbeafe' }; // Blue
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${screenWidth <= 320 ? 'p-2' : 'p-6'} ${className}`}>
      {/* Tank Container */}
      <div 
      ...
      >
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 pointer-events-none">
                ...
                {[...Array(screenWidth <= 320 ? 3 : 6)].map((_, i) => (
                  <div
                    ...
                    style={{
                      left: `${15 + i * 12}%`,
                      top: `${30 + Math.sin((animationPhase + i * 60) * Math.PI / 180) * 20}%`,
                      transform: `translateZ(${Math.cos((animationPhase + i * 45) * Math.PI / 180) * 15}px)`,
                      opacity: 0.3 + Math.sin((animationPhase + i * 30) * Math.PI / 180) * 0.2,
              ...
              <div 
                ...
                style={{
                  transform: `translateY(${Math.sin(animationPhase * Math.PI / 180) * 2}px)`,
        ...
        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2">
          <div 
            className={`${screenWidth <= 320 ? 'w-6 h-4' : 'w-8 h-6'} bg-gradient-to-b from-gray-700 to-gray-900 rounded-lg shadow-lg border border-gray-600`}
            style={{
              boxShadow: '0 4px 8px rgba(0,0,0,0.3), inset 0 1px 2px rgba(255,255,255,0.1)'
            }}
          >
            <div className={`${screenWidth <= 320 ? 'w-2 h-2 mt-0.5' : 'w-3 h-3 mt-1'} bg-blue-500 rounded-full mx-auto animate-pulse shadow-lg shadow-blue-500/50`} />
          </div>
        ...
        <div className="absolute -top-4 -right-2">
          <div 
            className={`${screenWidth <= 320 ? 'w-3 h-12' : 'w-4 h-16'} bg-gradient-to-b from-gray-600 to-gray-800 rounded-full shadow-lg`}
            style={{
          ...
          >
            ...
            <div 
              className={`absolute ${screenWidth <= 320 ? '-right-1 top-1 w-4 h-2' : '-right-2 top-2 w-6 h-3'} rounded-full border-2 ${
                pumpStatus === 'ON' 
                  ? 'bg-green-400 border-green-500 shadow-green-400/50' 
                  : 'bg-red-400 border-red-500 shadow-red-400/50'
              } shadow-lg flex items-center justify-center`}
            >
              <Zap className={`${screenWidth <= 320 ? 'w-1.5 h-1.5' : 'w-2 h-2'} ${pumpStatus === 'ON' ? 'text-white animate-pulse' : 'text-white'}`} />
            </div>
      ...
      <div className={`text-center ${screenWidth <= 320 ? 'space-y-2' : 'space-y-3'} w-full`}>
        {/* Main Water Level Display */}
        <div className="space-y-1">
          <div className={`${textSizes.mainValue} font-bold text-gray-800 drop-shadow-sm`}>
            {waterLevel}%
          </div>
          <div className={`${textSizes.title} font-semibold text-blue-600 uppercase tracking-widest`}>
            Water Level
          </div>
        </div>

        {/* Volume and Connection Info - Stack on very small screens */}
        <div className={`flex ${screenWidth <= 320 ? 'flex-col space-y-2' : 'items-center justify-center space-x-4'} ${textSizes.info}`}>
          <div className="flex items-center justify-center space-x-1">
            <Droplets className={`${screenWidth <= 320 ? 'w-3 h-3' : 'w-4 h-4'} text-blue-500`} />
          ...
          <div className="flex items-center justify-center space-x-1">
            {connectionStatus === 'connected' ? (
              <Wifi className={`${screenWidth <= 320 ? 'w-3 h-3' : 'w-4 h-4'} text-green-500`} />
            ) : (
              <WifiOff className={`${screenWidth <= 320 ? 'w-3 h-3' : 'w-4 h-4'} text-red-500`} />
            )}
        ...
        <div className="flex justify-center">
          <div 
            className={`${screenWidth <= 320 ? 'px-2 py-1' : 'px-4 py-2'} rounded-full ${textSizes.badge} font-semibold border-2 ${
              waterLevel < 20 
          ...
          >
            {waterLevel < 20 ? '🚨 Critical Level' :
             waterLevel < 40 ? '⚠️ Low Level' :
             waterLevel < 70 ? '💧 Normal Level' :
             '✅ Optimal Level'}
          </div>
        </div>
      </div>
